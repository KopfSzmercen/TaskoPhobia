...
# Adding Fluent Validator

To add validation for commands I’ll use Fluent Validator package.

Each command class will have its own validator class extending AbstractValidator class.

### Registering command validation

To make validating automatic for every command I’ll use CommandDispatcher that I already have in the app.

1. Get validator for command if exists

```csharp
var validator = scope.ServiceProvider.GetService<IValidator<TCommand>>();
```

1. If validator exists for a particular command use it.

```csharp
if (validator is not null) await validator.ValidateAndThrowAsync(command);
```

### Refactoring of Exception handling middleware

To adjust the exception handling middleware to new requirements, I’ll create

```csharp
public record ErrorsResponse(params Error[] Errors);
```

which will take any number of Errors which will allow to return many errors generated by fluent validator.

Next, I’ll handle another type of exception- ValidationException -in the middleware. I’ll add another nullable field to the Error record: Property which will allow me to include the name of the property which failed during validation.